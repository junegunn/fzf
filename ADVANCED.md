...
rg --color=always --line-number --no-heading --smart-case "${*:-}" |
  fzf --ansi \
      ...
      --bind 'enter:become(vim {1} +{2} < /dev/tty)'
```
...
- Instead of using shell script to process the final output of fzf, we use
  `become` action to turn fzf into a new process that opens the file with
  `vim` (`vim {1}`) and move the cursor to the line (`+{2}`).
...
- Instead of starting fzf in `rg ... | fzf` form, we start fzf without an
  ...
  even after `reload` is triggered.
    - This also allows us to remove `< /dev/tty` in `become(...)` binding
