...
_fzf_complete_ssh() {
  _fzf_complete '+m' "$@" < <(
    command cat <(cat ~/.ssh/config /etc/ssh/ssh_config 2> /dev/null | command grep -i '^host' | command grep -v 'HostKeyAlgorithms' | command grep -v '*' | awk '{for (i = 2; i <= NF; i++) print $1 " " $i}') \
