...
# Currently the code runs without errors, but it does work. (it does nothing)
...
def __fzf_generic_path_completion_nu [
  ...
  let candidates = if not ($compgen_cmd_name | is-empty) {
    ...
    # Use external `find` to replicate Zsh/Bash behavior closely
    let find_opts = ['-L', $root_dir_expanded] # Follow symlinks for root path
    let prune_opts = ['-name', '.git', '-prune', '-o', '-name', '.hg', '-prune', '-o', '-name', '.svn', '-prune', '-o']
    let type_opts = if ($suffix == '/') {
        # Directory completion: find only directories
        ['-type', 'd']
    } else {
    ...
    try {
      ...
      run-external "find" ...$all_find_opts | lines | str replace --regex '^\./' ''
    } catch {
      print -e $"Error running find in ($root_dir_expanded)"
      []
    }
  }
  ...
  let fzf_all_opts = $fzf_default_opts | append $fzf_opts_nu | append $completion_type_opts
